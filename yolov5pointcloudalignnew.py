...
def process_frame(frame):
    ...
    for detection in detections:
        ...
        if confidence > 0.7:  # Adjust this threshold as needed
            ...
            box_middle_x = x + w // 2
            box_middle_y = (y + h // 2) - 140 - 97.3-18.5 # 97.3 px of gripper distance, 70 px of detection box from conveyor
            middle_line_x = frame.shape[1] // 2
            middle_line_y = frame.shape[0] // 2
            
            pixcm = 9.75 # Pixel to CM value
            
            if abs(box_middle_x - middle_line_x) < 0.1:  # Adjust threshold as needed
                alignment_text = "aligned"
                print("Allign")
                distance = mid_point
                distance_y = ((middle_line_y - box_middle_y)/pixcm)*10
                alignment_text = f"move to mid ({distance: .2f}mm, Y: {distance_y:.2f}mm)"
                print(f"Move Right ({distance: .2f}mm from the middle line), Y: {distance_y:.2f}mm")
                
                command = "rostopic pub -1 /XYZcommanderGripperPosition geometry_msgs/Vector3 " + "{" + f"\'x: {distance:.2f}, y: {distance_y:.2f}, z: 30.0" + "}\'" #----------------------------------------------------------------------------------------------------------------------------------------------------
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        if int(class_label) == 0:
                            command2 = "rostopic pub -1 /GripperState std_msgs/Int16 "+ f"\"data: 0\" "
                        elif int(class_label) == 1:
                            command2 = "rostopic pub -1 /GripperState std_msgs/Int16 "+ f"\"data: 1\" "
                        result = subprocess.run(command2, shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)                        
                    else:
                ...
                distance = (((middle_line_x - box_middle_x)/pixcm)*-1)*factorRight*10+mid_point
                distance_y = ((middle_line_y - box_middle_y)/pixcm)*10
                alignment_text = f"move right ({distance: .2f}mm, Y:{distance_y:.2f}mm)"
                print(f"Move Right ({distance: .2f}mm from the middle line, Y:{distance_y:.2f}mm")
                ...
                distance = mid_point-abs((box_middle_x - middle_line_x)/pixcm)*10 *factorLeft
                distance_y = ((middle_line_y - box_middle_y)/pixcm)*10
                alignment_text = f"move left ({distance: .2f}mm, Y:{distance_y:.2f}cm)"
                print(f"Move Left ({distance: .2f}mm from the middle line), Y:{distance_y:.2f}cm")
